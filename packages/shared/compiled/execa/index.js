;(function () {
  var e = {
    ...
    580: function (e, t, n) {
      ...
      if (!processOk(r)) {
        ...
        e.exports.load = p
        var m = r.reallyExit
        var b = function processReallyExit(e) {
          if (!processOk(global.process)) {
            return
          }
          r.exitCode = e || 0
  ...
  !(function () {
    __nccwpck_require__.d = function (e, t) {
      for (var n in t) {
  ...
  !(function () {
    __nccwpck_require__.o = function (e, t) {
  ...
  !(function () {
    ...
    __nccwpck_require__.d(n, {
      ...
      execaCommand: function () {
        return execaCommand
      },
      execaCommandSync: function () {
    ...
    var o = require('process')
    var i = require('../cross-spawn')
    ...
    function npmRunPath(e = {}) {
      const {
        cwd: n = o.cwd(),
        path: r = o.env[pathKey()],
        execPath: i = o.execPath,
      } = e
      let a
    ...
    const makeError = ({
      ...
      const l = o === undefined ? undefined : b[o].description
      const p = r && r.code
      ...
      const x = y ? `${g}\n${r.message}` : g
      const h = [x, t, e].filter(Boolean).join('\n')
      if (y) {
    ...
    const normalizeStdio = (e) => {
      ...
      if (hasAlias(e)) {
        throw new Error(
          `It's not possible to provide \`stdio\` in combination with one of ${y
            .map((e) => `\`${e}\``)
            .join(', ')}`
        )
      }
    ...
    const shouldForceKill = (e, { forceKillAfterTimeout: t }, n) =>
      isSigterm(e) && t !== false && n
    const isSigterm = (e) =>
      e === x.constants.signals.SIGTERM ||
    ...
    var w = __nccwpck_require__(981)
    var _ = __nccwpck_require__(853)
    const handleInput = (e, t) => {
      if (t === undefined || e.stdin === undefined) {
        return
      }
      if (isStream(t)) {
        t.pipe(e.stdin)
    ...
    const I = ['then', 'catch', 'finally'].map((e) => [
      e,
      Reflect.getOwnPropertyDescriptor(v, e),
    ])
    const mergePromise = (e, t) => {
      for (const [n, r] of I) {
        const o =
          typeof t === 'function'
            ? (...e) => Reflect.apply(r.value, t(), e)
            : r.value.bind(t)
    ...
    function execa(e, t, n) {
      ...
      const handlePromise = async () => {
        const [{ error: e, exitCode: t, signal: n, timedOut: r }, c, u, d] =
          await getSpawnedResult(a, o.options, f)
        const p = handleOutput(o.options, c)
        const m = handleOutput(o.options, u)
        const b = handleOutput(o.options, d)
        if (e || t !== 0 || n !== null) {
          const c = makeError({
            error: e,
            exitCode: t,
            ...
            isCanceled:
              l.isCanceled ||
              (o.options.signal ? o.options.signal.aborted : false),
            killed: a.killed,
          })
          if (!o.options.reject) {
            return c
      ...
      handleInput(a, o.options.input)
      a.all = makeAllStream(a, o.options)
      return mergePromise(a, p)
    }
    function execaSync(e, t, n) {
      const o = handleArguments(e, t, n)
      const i = joinCommand(e, t)
      const s = getEscapedCommand(e, t)
      validateInputSync(o.options)
      let a
      try {
        a = r.spawnSync(o.file, o.args, o.options)
      } catch (e) {
        throw makeError({
          error: e,
          stdout: '',
      ...
      const c = handleOutput(o.options, a.stdout, a.error)
